...
Il pattern architetturale impiegato per gestire la visualizzazione della \textit{GameOverView} e della \textit{GameVictoryView} nel metodo \textit{draw()} della classe \textit{GameView} è Facade Pattern.
Quando il gioco giunge al termine, la schermata di fine gioco(quindi la View di vincita o di perdita) viene mostrata in modo condizionale attraverso i metodi forniti dal \textit{GameController}. Questo approccio consente di implicitare la complessità interna associata alla visualizzazione della View chiamata e di semplificare l'interazione con essa all'interno di un unico metodo della \textit{GameView}.

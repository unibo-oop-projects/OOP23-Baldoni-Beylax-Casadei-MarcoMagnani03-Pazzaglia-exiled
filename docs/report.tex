...
La \textit{GameOverView} e la \textit{GameVictoryView} sono state integrate all'interno della \textit{GameView}, la schermata principale definita in precedenza. All'interno di questa \textit{GameView} è anche presente il \textit{GameController}, che facilita il collegamento tra il Model e la View stessa. Grazie a questo Controller, è possibile determinare quando la schermata principale deve essere sostituita dalla schermate di vincita o perdita del gioco.
...
Il pattern architetturale impiegato per gestire la visualizzazione della \textit{GameOverView} e della \textit{GameVictoryView} nel metodo \textit{draw()} della classe \textit{GameView} è chiamato Facade Pattern.
